...
#     adv_images = torch.utils.data.DataLoader(adv_data["adv"].permute(0, 2, 3, 1), num_workers=0)
#     adv_labels = torch.utils.data.DataLoader(adv_data["label"], num_workers=0)
    
    adv_images = adv_data["adv"].permute(0, 2, 3, 1).numpy()
    ...
    train_adv_set = list(zip(transpose(pad(adv_images, 4)/255.),
        adv_labels))
    
#     train_adv_set = list(zip(adv_images,
#         adv_labels))
#     print(len(adv_data["label"]))
    train_adv_set_x = Transform(train_adv_set, transforms)
    train_adv_batches = Batches(train_adv_set_x, args.batch_size, shuffle=True, set_random_choices=True, num_workers=4)
